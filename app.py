...
def progress_callback(download_id, received_bytes, total_bytes):
    """Update progress in status dictionary"""
    if total_bytes:
        progress = min(100, math.floor(received_bytes / total_bytes * 100))
        if download_id in download_status:
            download_status[download_id]['progress'] = progress

async def async_download_media(channel, message_id, download_id):
    """Asynchronous download function with progress tracking"""
    try:
        async with TelegramClient(SESSION_FILE, API_ID, API_HASH) as client:
            # Create custom callback for this download
            callback = lambda r, t: progress_callback(download_id, r, t)
